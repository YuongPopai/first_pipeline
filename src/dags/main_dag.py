...
def get_report(ti):
    ...
    if not report_id:
        raise TimeoutError('Не удалось запросить report_id')
...
def upload_data_to_staging(filename, date, pg_table, pg_schema, ti):
    ...
    logger.info(f'{row_count} rows were inserted')
...
with DAG(
    ...
    update_izmer = []
    for file_path in ["sql/mart.d_item.sql", "sql/mart.d_customer.sql", "sql/mart.d_customer.sql"]:
        update_izmer.append(PostgresOperator(
            task_id=f'update_{file_path.split('/')[1]}',
